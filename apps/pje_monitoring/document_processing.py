...
class OCRProcessor:
    ...
    def extract_text_from_image(
        ...
        ocr_data = pytesseract.image_to_data(
            pil_img, lang=Config.TESSERACT_LANG, output_type=pytesseract.Output.DICT
        )
...
class OABExtractor(DocumentExtractor):
    ...
    def extract_from_text(self, text: str) -> Dict[str, Any]:
        ...
        # Extrair nome
        nome_patterns = [r"NOME[:\s]+([A-ZÀ-Ú\s]+)", r"ADVOGADO[:\s]+([A-ZÀ-Ú\s]+)"]
...
class DocumentClassifier:
    ...
    def classify_document(text: str) -> DocumentType:
        ...
        if re.search(
            r"CARTEIRA\s+DE\s+IDENTIDADE|REGISTRO\s+GERAL|SECRETARIA\s+DE\s+SEGURANÇA",
            text,
        ):
            return DocumentType.RG

        if re.search(
            r"CARTEIRA\s+NACIONAL\s+DE\s+HABILITAÇÃO|PERMISSÃO\s+PARA\s+DIRIGIR|DETRAN",
            text,
        ):
            return DocumentType.CNH

        if re.search(
            r"CADASTRO\s+DE\s+PESSOAS\s+FÍSICAS|CPF|MINISTÉRIO\s+DA\s+FAZENDA", text
        ):
            return DocumentType.CPF

        if re.search(
            r"ORDEM\s+DOS\s+ADVOGADOS\s+DO\s+BRASIL|OAB|IDENTIDADE\s+DE\s+ADVOGADO",
            text,
        ):
            return DocumentType.OAB

        if re.search(
            r"CONTA\s+DE\s+ENERGIA|CONTA\s+DE\s+ÁGUA|CONTA\s+DE\s+LUZ|COMPROVANTE\s+DE\s+RESIDÊNCIA|FATURA",
            text,
        ):
            return DocumentType.COMPROVANTE_RESIDENCIA
...
class DocumentTemplate:
    ...
    def render_pdf(
        ...
        try:
            ...
            if "signature_image" in context and os.path.exists(
                ...
                elements.append(Spacer(1, 6))
                elements.append(
                    Image(context["signature_image"], width=200, height=100)
                )
...
class DocumentGenerator:
    ...
    def generate_document(
        ...
        if template_path.endswith(".docx"):
            ...
            result = self.template_manager.render_docx(
            ...
            if output_format == "pdf" and output_path:
                pdf_path = (
                    ...
                    else f"{os.path.splitext(output_path)[0]}.pdf"
                )
            ...
            try:
                ...
                if "/AcroForm" in pdf.trailer["/Root"]:
                    ...
                    return self.template_manager.fill_pdf_form(
                        template_path, context, output_path
                    )
            except:
                pass
            ...
            raise ValueError(
                f"Formato de template não suportado: {os.path.splitext(template_path)[1]}"
            )
    ...
    def generate_procuracao(
        ...
        context = {
            ...
            "outorgante_nacionalidade": outorgante_data.get(
            ...
            "outorgado_nacionalidade": outorgado_data.get(
                "nacionalidade", "brasileiro(a)"
            ),
    ...
    def generate_contrato(
        ...
        context = {
            ...
            "advogado_nacionalidade": advogado_data.get(
                "nacionalidade", "brasileiro(a)"
            ),
        ...
        return self.generate_document(
            "contrato_advocaticio.docx", context, "pdf", output_path
        )
    ...
    def _convert_docx_to_pdf(self, docx_path: str, pdf_path: str) -> str:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "LibreOffice não disponível para conversão. Tentando alternativa..."
                )
            ...
            try:
                ...
                logger.warning(
                    "python-docx2pdf não disponível. Tentando outra alternativa..."
                )
            ...
            logger.error(
                "Não foi possível converter DOCX para PDF. Retornando documento DOCX."
            )
            return docx_path
        except Exception as e:
...
class GoogleFormIntegration(FormIntegration):
    ...
    def create_form(self, title: str, fields: List[Dict[str, Any]]) -> str:
        ...
        try:
            ...
            for field in fields:
                ...
                if field_type == "text":
                    item = {
                        ...
                        "textQuestion": {"paragraph": field.get("multiline", False)},
                    }
                    ...
                    item = {
                        ...
                        "choiceQuestion": {
                            "type": "RADIO",
                            "options": [
                                {"value": option} for option in field.get("options", [])
                            ],
                        },
                    }
                    ...
                    item = {"title": field.get("title", ""), "dateQuestion": {}}
                else:
                    continue
            ...
            update = {"requests": [{"createItem": item} for item in items]}
            ...
            for resp in response.get("responses", []):
                ...
                for question_id, answer in answers.items():
                    ...
                    if "textAnswers" in answer:
                        result[question_title] = answer["textAnswers"]["answers"][0][
                            "value"
                        ]
                    elif "choiceAnswers" in answer:
                        result[question_title] = [
                            choice["value"]
                            for choice in answer["choiceAnswers"]["answers"]
                        ]
                    elif "dateAnswers" in answer:
                        result[question_title] = answer["dateAnswers"]["answers"][0][
                            "value"
                        ]
    ...
    def get_form_responses_sheet(self, form_id: str) -> str:
        ...
        try:
            ...
            sheet = (
                self.sheets_service.spreadsheets().create(body=sheet_metadata).execute()
            )
...
class CustomFormIntegration(FormIntegration):
    ...
    def create_form(self, title: str, fields: List[Dict[str, Any]]) -> str:
        ...
        try:
            ...
            form_data = {"title": title, "fields": fields}
...
class DocumentProcessor:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.warning(
                f"Não foi possível inicializar integração com Google Forms: {e}"
            )
    ...
    ) -> Union[ExtractedData, List[ExtractedData]]:
        """
    ...
    def create_client_form(
        ...
        fields = [
            {"title": "Nome Completo", "type": "text", "required": True},
            {"title": "CPF", "type": "text", "required": True},
            {"title": "RG", "type": "text", "required": True},
            {"title": "Data de Nascimento", "type": "date", "required": True},
            {
                "title": "Estado Civil",
                "type": "choice",
                "options": [
            ...
            {"title": "Profissão", "type": "text", "required": True},
            {
            ...
            {"title": "CEP", "type": "text", "required": True},
            {"title": "Cidade", "type": "text", "required": True},
            {
                "title": "Estado",
                "type": "choice",
                "options": [
            ...
            {"title": "Telefone", "type": "text", "required": True},
        ...
        try:
            ...
            if use_google_forms and self.google_form_integration:
                ...
                return {"form_id": form_id, "form_url": form_url, "type": "custom_form"}
        except Exception as e:
    ...
    def create_case_form(
        ...
        fields = [
            ...
            {"title": "Vara/Tribunal", "type": "text", "required": False},
            {"title": "Parte Contrária", "type": "text", "required": True},
            {"title": "Valor da Causa", "type": "text", "required": True},
            {"title": "Honorários Combinados", "type": "text", "required": True},
            {
    ...
    def process_form_data(
        self, form_id: str, form_type: str = "custom_form"
    ) -> List[Dict[str, Any]]:
        """
        ...
        for field, value in form_data.items():
            ...
            if "CPF" in field:
                ...
                if len(normalized_value) == 11:
                    ...
                    validated_data["cpf"] = (
                        f"{normalized_value[:3]}.{normalized_value[3:6]}.{normalized_value[6:9]}-{normalized_value[9:]}"
                    )
                else:
                ...
                if isinstance(value, dict) and "year" in value:
                    ...
                    validated_data["data_nascimento"] = (
                ...
                if len(normalized_value) == 8:
                    validated_data["cep"] = (
                        f"{normalized_value[:5]}-{normalized_value[5:]}"
                    )
                else:
                ...
                if len(normalized_value) == 11:  # Celular com DDD
                    validated_data["telefone"] = (
                        f"({normalized_value[:2]}) {normalized_value[2:7]}-{normalized_value[7:]}"
                    )
                elif len(normalized_value) == 10:  # Fixo com DDD
                    validated_data["telefone"] = (
                        f"({normalized_value[:2]}) {normalized_value[2:6]}-{normalized_value[6:]}"
                    )
                else:
    ...
    def extract_client_data_from_documents(
        self, documents: List[Union[str, BinaryIO]]
    ) -> Dict[str, Any]:
        """
        ...
        for document in documents:
            try:
                ...
                for field, value in extracted_data.fields.items():
                    ...
                    if (
                        field not in client_data
                        or extracted_data.confidence
                        > client_data.get("_confidence", {}).get(field, 0)
                    ):
                ...
                client_data["_document_types"].append(
                    extracted_data.document_type.value
                )
    ...
    def generate_documents_from_client_data(
        self,
            ...
            try:
                if doc_type == "procuracao":
                    ...
                    output_path = (
                        os.path.join(
                            output_dir,
                            f"procuracao_{client_data.get('cpf', 'cliente')}.pdf",
                        )
                        if output_dir
                        else None
                    )
                    ...
                    output_path = (
                        os.path.join(
                            output_dir,
                            f"contrato_{client_data.get('cpf', 'cliente')}.pdf",
                        )
                        if output_dir
                        else None
                    )
                    ...
                    output_path = (
                        os.path.join(
                            output_dir,
                            f"declaracao_{client_data.get('cpf', 'cliente')}.pdf",
                        )
                        if output_dir
                        else None
                    )
    ...
    def send_documents_for_signature(
        self,
    ...
    def check_signature_status(
        self, signature_id: str, signature_service_url: str, api_key: str
    ) -> Dict[str, Any]:
        """
...
def process_client_onboarding(
    ...
    try:
        ...
        client_data = document_processor.extract_client_data_from_documents(
            client_documents
        )

        # 2. Gerar documentos necessários
        generated_documents = document_processor.generate_documents_from_client_data(
            client_data, ["procuracao", "contrato"], output_dir
        )

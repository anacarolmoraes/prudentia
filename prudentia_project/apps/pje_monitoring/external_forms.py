...
class Config:
    ...
    NOTIFICATION_EMAIL = os.getenv("NOTIFICATION_EMAIL", "notificacoes@prudentia.com.br")
    NOTIFICATION_WHATSAPP_ENABLED = os.getenv("NOTIFICATION_WHATSAPP_ENABLED", "true").lower() == "true"
    WHATSAPP_API_URL = os.getenv("WHATSAPP_API_URL", "https://api.whatsapp.com/v1/messages")
...
class ExternalForm(models.Model):
    ...
    form_type = models.CharField(max_length=20, choices=[(t.value, t.name) for t in FormType])
    purpose = models.CharField(max_length=30, choices=[(p.value, p.name) for p in FormPurpose])
    fields = models.JSONField()
    external_id = models.CharField(max_length=255, null=True, blank=True)  # ID no sistema externo (Google Forms)
    external_url = models.URLField(null=True, blank=True)  # URL no sistema externo
    access_token = models.CharField(max_length=64, unique=True)  # Token para acesso ao formulário
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.expires_at:
            self.expires_at = timezone.now() + datetime.timedelta(days=Config.FORM_EXPIRY_DAYS)
    ...
    def is_available(self):
        ...
        return self.is_active and not self.is_expired and not self.has_reached_max_responses

class FormResponseModel(models.Model):
    """Modelo para armazenar respostas de formulários."""
    
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    form = models.ForeignKey(ExternalForm, on_delete=models.CASCADE, related_name='responses')
    ...
    def add_generated_document(self, document_type: str, document_path: str, document_id: Optional[str] = None):
...
class FormService:
    ...
    def process_form_response(self, 
                            ...
                            ip_address: Optional[str] = None,
                            user_agent: Optional[str] = None) -> FormResponseModel:
        """
    ...
    def _send_response_notifications(self, form: ExternalForm, response: FormResponseModel):
        """
    ...
    def _generate_documents_from_response(self, form: ExternalForm, response: FormResponseModel):
        """
        ...
        logger.info(f"Gerando documentos para resposta {response.id} do formulário {form.id}")
...
class CustomFormService(FormService):
    ...
    def create_form(self, 
                   ...
                   auto_generate_documents: bool = False) -> ExternalForm:
        """
    ...
    def update_form(self, 
                   ...
                   max_responses: Optional[int] = None,
                   document_template_id: Optional[str] = None,
                   auto_generate_documents: Optional[bool] = None) -> ExternalForm:
        """
        ...
        if form.form_type != FormType.CUSTOM.value:
            raise ValueError(f"O formulário {form_id} não é um formulário personalizado")
...
class GoogleFormService(FormService):
    ...
    def create_form(self, 
        ...
        try:
            ...
            for i, field_data in enumerate(fields):
                ...
                if field_type == FieldType.TEXT.value or field_type == FieldType.TEXTAREA.value:
                    item['textQuestion'] = {
                    ...
                    item['choiceQuestion'] = {
                        'type': 'CHECKBOX',
                        'options': [{'value': opt.get('label', opt.get('value', ''))} for opt in field_options]
                    }
                elif field_type == FieldType.RADIO.value or field_type == FieldType.SELECT.value:
                    item['choiceQuestion'] = {
                        'type': 'RADIO',
                        'options': [{'value': opt.get('label', opt.get('value', ''))} for opt in field_options]
                    }
    ...
    def update_form(self, 
        ...
        try:
            ...
            if fields is not None:
                ...
                existing_form = self.forms_service.forms().get(formId=form.external_id).execute()
    ...
    def get_form_responses(self, form_id: str) -> List[FormResponseModel]:
        ...
        try:
            ...
            db_responses = FormResponseModel.objects.filter(form=form).order_by('-submitted_at')
            
            # Obter respostas do Google Forms
            try:
                google_responses = self.forms_service.forms().responses().list(
                    formId=form.external_id
                ).execute()
                ...
                for response_data in google_responses.get('responses', []):
                    ...
                    if not FormResponseModel.objects.filter(form=form, data__responseId=response_id).exists():
                        # Obter detalhes da resposta
                        response_detail = self.forms_service.forms().responses().get(
                            formId=form.external_id,
                            responseId=response_id
                        ).execute()
                        
                        # Processar dados da resposta
                        processed_data = self._process_google_form_response(form, response_detail)
                        
                        # Criar resposta no banco de dados
                        FormResponseModel.objects.create(
                            form=form,
                            data=processed_data,
                            submitted_at=datetime.datetime.fromisoformat(response_detail.get('lastSubmittedTime')),
    ...
    def _process_google_form_response(self, form: ExternalForm, response_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            google_form = self.forms_service.forms().get(formId=form.external_id).execute()
            ...
            for item in items:
                ...
                if 'textQuestion' in item:
                    ...
                    id_to_type[item_id] = 'checkbox' if choice_type == 'CHECKBOX' else 'radio'
            ...
            for question_id, answer_data in answers.items():
                ...
                if question_type == 'text':
                    ...
                    processed_data[key] = [answer.get('value', '') for answer in choice_answers]
                    ...
                    if date_answers:
                        ...
                        if date_value:
                            processed_data[key] = f"{date_value.get('day', 1):02d}/{date_value.get('month', 1):02d}/{date_value.get('year', 2023)}"
                elif question_type == 'time':
                    time_answers = answer_data.get('timeAnswers', {}).get('answers', [])
                    if time_answers:
                        time_value = time_answers[0].get('value', {})
                        if time_value:
                            processed_data[key] = f"{time_value.get('hours', 0):02d}:{time_value.get('minutes', 0):02d}"
                elif question_type == 'scale':
                    scale_answers = answer_data.get('scaleAnswers', {}).get('answers', [])
                    if scale_answers:
...
def create_form_service(form_type: FormType = None, form_id: Optional[str] = None) -> FormService:
    """
...
def create_client_registration_form(user, use_google_forms: bool = False) -> ExternalForm:
    """

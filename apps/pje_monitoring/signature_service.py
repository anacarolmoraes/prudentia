...
class Settings:
    SECRET_KEY = os.getenv(
        "SECRET_KEY", "prudentia_signature_secret_key_change_in_production"
    )
    ...
    DOCUMENT_STORAGE_PATH = os.getenv(
        "DOCUMENT_STORAGE_PATH", "/tmp/prudentia/documents"
    )
...
class Signature(Base):
    ...
    )  # ID do usuário que assinou (null se pendente)
    ...
    )  # Página do documento onde a assinatura aparece
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class BlockchainService:
    ...
    async def register_document(
        self, document_hash: str, signers: List[str]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(
                f"Document hash {document_hash} registered on blockchain with tx {tx_hash}"
            )
    ...
    async def verify_document(self, document_hash: str, tx_hash: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "tx_hash": tx_hash,
                "timestamp": int(time.time())
                - 3600,  # Simulamos que foi registrado há 1 hora
...
def create_signature_jwt(signature_id: str, document_id: str) -> str:
    ...
    payload = {"sub": signature_id, "doc": document_id, "exp": expiry.timestamp()}
...
class SignatureService:
    ...
    async def create_document(
        self, user_id: str, document_data: DocumentCreate, file: UploadFile
    ) -> Document:
        """
        ...
        if not file.content_type == "application/pdf":
            raise HTTPException(
                status_code=HTTP_400_BAD_REQUEST, detail="Only PDF files are supported"
            )
    ...
    def add_signatories(
        self, document_id: str, signatories: List[SignatureCreate]
    ) -> List[Signature]:
        """
    ...
    async def sign_document(
        ...
        # Buscar a assinatura
        signature = (
            self.db.query(Signature).filter(Signature.id == signature_id).first()
        )
        if not signature:
            raise HTTPException(
                status_code=HTTP_404_NOT_FOUND,
        ...
        if signature.signature_token != token:
            raise HTTPException(
                status_code=HTTP_400_BAD_REQUEST, detail="Invalid signature token"
            )
        ...
        # Buscar o documento
        document = (
            self.db.query(Document).filter(Document.id == signature.document_id).first()
        )
        if not document:
            raise HTTPException(
                status_code=HTTP_404_NOT_FOUND,
        ...
        if document.expires_at and document.expires_at < datetime.utcnow():
            ...
            self.db.commit()
            raise HTTPException(
                status_code=HTTP_400_BAD_REQUEST, detail="Document has expired"
            )
        ...
        if pending_signatures == 0:
            ...
            signers = [
                sig.signer_email
                for sig in self.db.query(Signature)
                .filter(
                    Signature.document_id == document.id,
                    Signature.status == SignatureStatus.SIGNED.value,
                )
            ...
            blockchain_result = await self.blockchain.register_document(
                document.file_hash, signers
            )
            ...
            signature_info.append(
                {
                    ...
                    "signature_date": (
                        sig.signature_date.isoformat() if sig.signature_date else None
                    ),
...
def create_signature_router(get_db_session):
    ...
    @router.post(
        "/documents", response_model=DocumentResponse, status_code=HTTP_201_CREATED
    )
    async def create_document(
    ...
    @router.post(
        "/documents/{document_id}/signatories", response_model=List[SignatureResponse]
    )
    def add_signatories(
        document_id: str,
    ...
    async def verify_document(document_id: str, db: Session = Depends(get_db_session)):
        """Verifica a autenticidade e integridade de um documento assinado."""
        service = SignatureService(db)
    ...
    def get_document(document_id: str, db: Session = Depends(get_db_session)):
    ...
    def get_signature_url(signature_id: str, db: Session = Depends(get_db_session)):
        """Gera uma URL para assinatura."""
        service = SignatureService(db)
    ...
    def download_document(document_id: str, db: Session = Depends(get_db_session)):

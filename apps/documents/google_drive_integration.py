...
class DriveFile(models.Model):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="drive_files"
    )
...
class DriveAuthService:
    ...
    def get_credentials_flow(self, redirect_uri: str) -> Flow:
        ...
        try:
            ...
            flow = Flow.from_client_config(
                client_config, scopes=DriveConfig.SCOPES, redirect_uri=redirect_uri
            )
            return flow
    ...
    def revoke_credentials(self, user):
        ...
        try:
            ...
            logger.warning(
                f"Não há credenciais para revogar para o usuário {user.email}"
            )
        except Exception as e:
...
class DriveService:
    ...
    def create_folder(self, name: str, parent_id: Optional[str] = None) -> Dict:
        ...
        """
        try:
            file_metadata = {"name": name, "mimeType": DriveConfig.MIME_TYPES["folder"]}
    ...
    def update_file(
        ...
        try:
            if file_path:
                ...
                if not mime_type:
                    raise ValueError(
                        "mime_type é obrigatório quando content é fornecido"
                    )
    ...
    def create_shortcut(
        self, target_id: str, name: str, parent_id: Optional[str] = None
    ) -> Dict:
        """
...
class DriveSyncService:
    ...
    def _start_sync_log(self):
        ...
        self.sync_log = DriveSyncLog.objects.create(
            user=self.user, status="in_progress"
        )
        ...
        try:
            ...
            if root_results:
                ...
                root_folder = self.drive_service.create_folder(
                    DriveConfig.DEFAULT_FOLDERS["root"]
                )
                root_id = root_folder["id"]
                folder_ids["root"] = root_id
        ...
        try:
            ...
            if file_results:
                ...
                drive_file_info = self.drive_service.update_file(
                    file_results[0]["id"], file_path=local_path
                )
                action = "updated"
            else:
                # Fazer upload de novo arquivo
                drive_file_info = self.drive_service.upload_file(
                    local_path, folder_id, file_name
                )
                action = "uploaded"
            ...
            drive_file, created = DriveFile.objects.update_or_create(
                ...
                defaults={
                    ...
                    "is_folder": drive_file_info["mimeType"]
                    == DriveConfig.MIME_TYPES["folder"],
    ...
    def sync_content_to_drive(
        ...
        try:
            ...
            if file_results:
                # Atualizar arquivo existente
                drive_file_info = self.drive_service.update_file(
                    file_results[0]["id"], content=content, mime_type=mime_type
                )
                action = "updated"
            else:
                # Fazer upload de novo arquivo
                drive_file_info = self.drive_service.upload_file_content(
                    content, file_name, mime_type, folder_id
                )
                action = "uploaded"
    ...
    def sync_file_from_drive(
        self, drive_id: str, local_path: Optional[str] = None
    ) -> Union[str, bytes]:
        """
    ...
    def backup_to_drive(
        ...
        try:
            ...
            drive_file = self.sync_content_to_drive(
                content, file_name, "application/zip", "backup"
            )
    ...
    def share_with_client(
        self, drive_id: str, client_email: str, message: Optional[str] = None
    ) -> Dict:
        """
        ...
        try:
            ...
            permission = self.drive_service.share_file(
                ...
                message=message
                or "Um documento foi compartilhado com você pelo escritório de advocacia.",
            )
...
def start_drive_auth_flow(request, redirect_uri):
    ...
    request.session["drive_auth_flow"] = {"state": flow.state}
...
def complete_drive_auth_flow(request, code, state):
    ...
    # Verificar estado
    if (
        "drive_auth_flow" not in request.session
        or request.session["drive_auth_flow"].get("state") != state
    ):
    ...
    try:
        auth_service = DriveAuthService()
        flow = auth_service.get_credentials_flow(
            request.build_absolute_uri("/drive/auth/callback/")
        )
        flow.state = state

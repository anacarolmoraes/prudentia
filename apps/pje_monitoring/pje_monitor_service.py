...
class Config:
    ...
    PALAVRAS_CHAVE_URGENTE = [
        "liminar", "antecipação de tutela", "urgente", "mandado de segurança",
        "habeas corpus", "prazo", "intimação", "citação", "audiência", "sentença",
        "acórdão", "decisão", "despacho", "julgamento", "penhora", "bloqueio"
    ]
...
class Advogado(models.Model):
    ...
    intervalo_monitoramento = models.IntegerField(default=Config.MONITORAMENTO_INTERVALO_PADRAO)
...
class PublicacaoMonitorada(models.Model):
    ...
    id = models.AutoField(primary_key=True)
    processo = models.ForeignKey(Processo, on_delete=models.CASCADE, related_name="publicacoes")
...
class LogMonitoramento(models.Model):
    ...
    id = models.AutoField(primary_key=True)
    advogado = models.ForeignKey(Advogado, on_delete=models.CASCADE, related_name="logs")
...
class PJeMonitorService:
    ...
    def agendar_monitoramento_para_advogado(self, advogado):
        ...
        if advogado.ultima_verificacao:
            ...
            horas_desde_ultima = (timezone.now() - advogado.ultima_verificacao).total_seconds() / 3600
            if horas_desde_ultima < advogado.intervalo_monitoramento:
                # Ainda não é hora de verificar novamente
                proxima_verificacao = advogado.ultima_verificacao + timedelta(hours=advogado.intervalo_monitoramento)
        ...
        logger.info(f"Monitoramento agendado para advogado {advogado.id} em {delay_seconds/60:.1f} minutos")
    ...
    async def buscar_publicacoes_para_advogado(self, advogado_id):
        ...
        if advogado.ultima_verificacao:
            ...
            data_inicio = data_fim - timedelta(days=Config.MONITORAMENTO_DIAS_RETROATIVOS)
        ...
        try:
            ...
            logger.error(f"Erro ao buscar publicações para advogado {advogado_id}: {str(e)}")
    ...
    def notificar_publicacao(self, publicacao, advogado):
        ...
        dados = {
            ...
            'url_processo': publicacao.url_processo or f"https://prudentia.com.br/processo/{publicacao.processo.numero_processo}/",
        ...
        if Config.NOTIFICACAO_WHATSAPP_ATIVO and advogado.notificacao_whatsapp and advogado.whatsapp:
    ...
    def _enviar_email_notificacao(self, dados):
        ...
        try:
            ...
            logger.info(f"Email enviado para {dados['advogado'].email} sobre publicação {dados['publicacao'].id}")
        except Exception as e:
    ...
    def _enviar_whatsapp_notificacao(self, dados):
        ...
        try:
            ...
            prioridade_emoji = "🔴" if dados['prioridade_texto'] == "URGENTE" else "🟠" if dados['prioridade_texto'] == "Alta" else "🟡"
            ...
            payload = {
                "phone": dados['advogado'].whatsapp.replace("+", "").replace("-", "").replace(" ", ""),
                "message": mensagem
            }
            ...
            if response.status_code != 200:
                ...
                logger.info(f"WhatsApp enviado para {dados['advogado'].whatsapp} sobre publicação {dados['publicacao'].id}")
...
def monitorar_publicacoes_advogado(self, advogado_id):
    ...
    try:
        ...
        resultado = loop.run_until_complete(service.buscar_publicacoes_para_advogado(advogado_id))
        
        # Processar publicações encontradas
        total, novas = service.processar_publicacoes(advogado_id, resultado)
        
        celery_logger.info(f"Monitoramento concluído para advogado {advogado_id}: {total} publicações, {novas} novas")
        ...
        celery_logger.error(f"Erro no monitoramento do advogado {advogado_id}: {str(e)}")
        
        # Tentar novamente em caso de falha
        try:
            self.retry(countdown=60 * 30)  # Tentar novamente em 30 minutos
        except Exception as retry_error:
            celery_logger.error(f"Falha definitiva no monitoramento do advogado {advogado_id}: {str(retry_error)}")
